...
if (-not $NoProvenance) {
  ...
  try {
    ...
    # $ErrorActionPreference='Stop'. Without this check a failed branch creation
    # (invalid -Label characters, an index.lock, an existing branch) would fall
    # through and commit PROVENANCE.md onto whatever was checked out --
    # possibly master, which must stay a clean mirror of upstream.
    if ($LASTEXITCODE -ne 0) {
      Fail "Could not create branch '$provBranch' (git exit $LASTEXITCODE). Refusing to continue: the provenance commit would land on '$(git rev-parse --abbrev-ref HEAD)'."
    }
...
| Host compiler | MSVC (via vcvars64; see the build log for the exact version) |
| CUDA toolkit | $(if ($CpuOnly) { 'n/a (CPU-only build)' } else { try { ((& (Join-Path $CudaToolkit 'bin\nvcc.exe') --version 2>&1 | Select-String 'release').Line.Trim()) } catch { 'unknown' } }) |
| GPUs | $(try { ((nvidia-smi --query-gpu=name,compute_cap --format=csv,noheader) -join '; ') } catch { 'n/a (nvidia-smi unavailable)' }) |
...
"@
    ...
    # The compiler consumes the WORKING TREE, so if it is dirty the pushed ref
    # must contain those changes too -- otherwise the branch claims to describe
    # a binary it cannot reproduce. Stage everything in that case.
    if ($dirty -gt 0) {
      Write-Host "staging $dirty working-tree change(s) so the ref matches what will be compiled" -ForegroundColor Yellow
