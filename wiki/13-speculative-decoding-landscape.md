...
| | |
| --- | --- |
| **What the report adds** | DFlash2 exists and its GGUF sizes are exact to the byte (§13.2). ⚠️ **Corrected by [chapter 15 §15.1](15-mmvq-ampere-experiment.md):** this row used to say the baseline binary "already accepts it". It accepts the **flag**, not the **checkpoint** — `src/models/dflash.cpp` builds no convolution or selector tensors, so a DFlash2 GGUF fails in `done_getting_tensors` with `wrong number of tensors`. DFlash2 needs [PR #27342](https://github.com/ggml-org/llama.cpp/pull/27342) built from source, and for Qwen3.8-27B no v1 drafter exists as a fallback. The prefill/decode phase split as an organising idea (§15, §22). And above all **§16.2** — the prefix-reuse objection to prefill-side selection, which is the most valuable claim in all three audited documents and which our own data upgrades from "may" to "does" (§13.7) |
